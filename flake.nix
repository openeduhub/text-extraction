{
  ...
  outputs = { self, nixpkgs, flake-utils, ... }:
    let
      ...
      get-text-extraction-lib = py-pkgs: py-pkgs.buildPythonPackage {
      ...
      get-text-extraction-app = pkgs: py-pkgs:
        ...
        python-lib = (final: prev: {
          pythonPackagesExtensions = prev.pythonPackagesExtensions ++ [
            (python-final: python-prev: {
              text-extraction = get-text-extraction-lib python-final;
            })
          ];
        });
        # add the webservice to nixpkgs
        default = (final: prev: {
          text-extraction = self.outputs.packages.${final.system}.default;
        });
      };
    } //
